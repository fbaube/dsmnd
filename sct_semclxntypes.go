...
// SemanticClxnType (SCT_*) characterizes the semantics of data 
// structures (SCT_... TABLE UTREE OTREE STREE) that are more
// complex than simple lists. ENUME is included because it
// (a) uses a Datum and (b) also can implement the hierarchical
// naming scheme for faceted metadata and other enumerations.
//  - Symbol names are `"SCT_"` + FIVE U.C. letters
...
const(
        ...
        SCT_STREE = SemanticClxnType("stree") // e.g. sorted tree 
)
...
{BDT_CLXN.DT(), SCT_STREE.S(), "Sortd tree", "Sorted tree of comparable children (XML mixed content)"},
}
